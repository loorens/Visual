...
void SredniaWariancja(float * tab, int n);
...
void ClimbingFromRandomPoint(Item* items, unsigned int &globalMaximumValue,
	...
	unsigned int &iteriationMaximum,
	unsigned int numberOfSteps);
void ControlledAnelling(Item* items,
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (int i = 0; i < 20; i++)
	{
		GlobalMaximum = GlobalMaximumValue = IterationMaximum = 0;
		timeStart = time(NULL);
		ClimbingFromRandomPoint(items, GlobalMaximumValue, GlobalMaximum, IterationMaximum, 100);
		timeOfSearch = time(NULL) - timeStart;
		printf("%i. ClimbingFromRandomPoint:\nMaximum = %u\nMax value = %u\nR = %u\nIteration = %u\nSearch time = %i[s]\n\n", i + 1, GlobalMaximum, GlobalMaximumValue, 589 - GlobalMaximumValue, IterationMaximum, timeOfSearch);
		wart2[i] = GlobalMaximumValue;
	}
	SredniaWariancja(wart2, 20);
	...
	for (int i = 0; i < 20; i++)
	{
		GlobalMaximum = GlobalMaximumValue = IterationMaximum = 0;
		timeStart = time(NULL);
		ControlledAnelling(items, GlobalMaximumValue, GlobalMaximum, IterationMaximum, 100);
		timeOfSearch = time(NULL) - timeStart;
		printf("%i. ControlledAnelling:\nMaximum = %u\nMax value = %u\nR = %u\nIteration = %u\nSearch time = %i[s]\n\n", i + 1, GlobalMaximum, GlobalMaximumValue, 589 - GlobalMaximumValue, IterationMaximum, timeOfSearch);
		wart3[i] = GlobalMaximumValue;
	}
	SredniaWariancja(wart3, 20);
...
void ControlledAnelling(Item* items, 
	...
	unsigned int numberOfSteps)
{
	unsigned int weight, value, newWeight, newValue ,T = numberOfSteps;
	double kb = 0.3;
	unsigned int startingPoint = distribution(rd);
	unsigned int newPoint = 0;
	int changeBit = 0;
	for (unsigned int i = 0; i < numberOfSteps; i++)
	{
		...
		if (newWeight <= MAX_KNAPSACK_WEIGHT && newValue > value)
		{
			globalMaximumValue = newValue;
			globalMaximum = startingPoint = newPoint;
			iteriationMaximum = i;
